...
export const verifyPharmacyStockService = async (batch_no, sub_stock_id) => {
  if (!batch_no || !sub_stock_id) {
    throw new Error("Batch or substock number is required");
  }
  ...
  const [affectedRows] = await sequelize.query(
    ...
    WHERE batch_no = ?
      AND verification = 'waiting' AND sub_stock_id = ?
    `,
    {
      replacements: [batch_no, sub_stock_id],
