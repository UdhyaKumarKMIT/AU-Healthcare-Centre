...
export const clearMedicineBatchService = async (
  ...
  const [insertResult] = await sequelize.query(
    ...
    SELECT
      ...
      ?
    FROM pharmacy_stock
    WHERE batch_no = ? and sub_stock_id = ?;
    `,
    {
      replacements: [pharmacist_code, batch_id, sub_stock_id],
      transaction
    }
  );
  ...
  await sequelize.query(
    `DELETE FROM pharmacy_stock WHERE batch_no = ? and sub_stock_id = ?`,
    {
      replacements: [batch_id, sub_stock_id],
      transaction
    }
  );
