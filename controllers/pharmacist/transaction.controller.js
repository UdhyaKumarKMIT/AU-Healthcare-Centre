...
export const verifyPharmacyStock = async (req, res) => {
  try {
    const { batch_no, sub_stock_id } = req.body;

    if (!batch_no || !sub_stock_id) {
      return res.status(400).json({ message: "Batch or substock number is required" });
    }

    await verifyPharmacyStockService(batch_no, sub_stock_id);
